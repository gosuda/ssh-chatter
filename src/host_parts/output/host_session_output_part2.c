...
void session_process_pending_sink(session_ctx_t *ctx)
{
    ...
    if (!used_incremental_redraw) {
        /*
         * SSH clients are sensitive to full-screen clear redraws and can show
         * visible flicker. When incremental diffing is unavailable, append only
         * the newest history line for SSH while keeping full redraw for telnet.
         */
