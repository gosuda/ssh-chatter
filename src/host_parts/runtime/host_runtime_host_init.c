...
static bool host_ai_chat_message_mentions_persona(const char *text)
{
    if (text == nullptr || text[0] == '\0') {
        return false;
    }
    return host_ai_chat_message_mentions(text, "kaka") ||
           host_ai_chat_message_mentions(text, "dada") ||
           host_ai_chat_message_mentions(text, "카카") ||
           host_ai_chat_message_mentions(text, "다다");
}
                                        ...
                                        const chat_history_entry_t *entry)
{
    ...
    if (!success) {
        ...
        if (host_ai_chat_message_mentions_persona(entry->message)) {
            if (korean) {
                snprintf(reply, sizeof(reply),
                         (persona == AI_CHAT_BOT_KAKA)
                             ? "카카 여기 있어! 지금 AI 응답 연결이 잠깐 불안정해."
                             : "다다 보고 있어. 잠깐 멍해졌지만 곧 돌아올게.");
            } else {
                snprintf(reply, sizeof(reply),
                         (persona == AI_CHAT_BOT_KAKA)
                             ? "kaka here! AI backend looks busy for a moment."
                             : "dada is here... backend hiccup. trying again soon.");
            }
            success = true;
        } else {
            return;
