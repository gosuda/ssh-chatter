/**
 * @file webssh_client.c
 * @desc File-level documentation for webssh_client.c, describing its role
                                      ...
                                      client_manager_t *manager)
{
    ...
    webssh_client_t *client =
        (webssh_client_t *)sshc_gc_calloc(1U, sizeof(webssh_client_t));
    if (client == nullptr) {
        return nullptr;
    }
    ...
    if (!client_manager_register(manager, &client->connection)) {
        ttak_mutex_destroy(&client->lock);
        sshc_gc_free(client);
        return nullptr;
    }
